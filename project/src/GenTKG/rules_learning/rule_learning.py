...
class Rule_Learner(object):
    def __init__(self, edges, id2relation, inv_relation_id, dataset):
       ...
       # path_workspace = os.path.join(current_dir, "../../../data/raw/", dataset)
    ...
    def create_rule(self, walk, temporal_deltas):
        """
        Create a rule given a cyclic temporal random walk.
        ...
        Parameters:
            walk (dict): cyclic temporal random walk
                        {"entities": list, "relations": list, "timestamps": list}
            temporal_deltas (list): list of temporal deltas between each step in the walk
        ...
        rule["temporal_deltas"] = temporal_deltas[::-1]  # Reverse to match body order
    ...
    def save_rules(self, dt, rule_lengths, num_walks, transition_distr, seed):
        ...
        for rel in self.rules_dict:
            for rule in self.rules_dict[rel]:
                rules_list.append({
                    "head_rel": int(rule["head_rel"]),  # Convert to native Python int
                    "body_rels": [int(rel) for rel in rule["body_rels"]],
                    "temporal_deltas": [int(delta) for delta in rule["temporal_deltas"]],
                    "var_constraints": rule["var_constraints"],
                    "conf": float(rule["conf"]),  # Convert to native Python float
                    "rule_supp": int(rule["rule_supp"]),
                    "body_supp": int(rule["body_supp"]),
                })
        ...
        filename = filename.replace(" ", "")
        with open(self.output_dir + filename, "w", encoding="utf-8") as fout:
            json.dump(rules_list, fout, indent=2)
