...
def learn_rules(i, num_relations): #include temp. delta
    if seed:
        np.random.seed(seed)
    ...
    for k in relations_idx:
        ...
        for length in rule_lengths:
            ...
            for _ in range(num_walks):
                ...
                if walk_successful:
                    temporal_deltas = [
                        walk["timestamps"][i + 1] - walk["timestamps"][i]
                        for i in range(len(walk["timestamps"]) - 1)
                    ]
                    # Debugging print
                    print(f"Timestamps: {walk['timestamps']}, Temporal Deltas: {temporal_deltas}")
                    rl.create_rule(walk, temporal_deltas)
            it_end = time.time()
            it_time = round(it_end - it_start, 6)
            print(f"Process {i}: relation {k - relations_idx[0] + 1}/{len(relations_idx)}, length {length}: {it_time} sec")
